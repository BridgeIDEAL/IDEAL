...
public class ArchiveLogManager : MonoBehaviour
{
    ...
    private string[] archiveTexts = {
        ...
        "$attempts번 실종자는 갑자기 ‘교무실에서 왜 사람이...  분명 아무도 없었는데’라는 말을 반복하던 중 신호가 끊김. 아직 정확한 생사는 불명.",
        "$attempts번 실종자는 목이 뒤로 꺾인 여학생과 조우한 후로 신호가 끊김. 이후 머리 없이 척추가 뒤로 꺾인 시체가 발견되었는데, 해당 실종자로 추정됨.",
        "$attempts번 실종자는 교무실에서 열쇠를 집은 뒤 나오려는 순간 신호가 끊김. 간신히 복원한 기록에서는 여성으로 추정되는 비명소리가 들린 것 외에는 실종자의 흔적을 찾을 수 없었음.",
        ...
        "$attempts번 실종자는 교실에 들어간 후 칠판에 뭔가 있다는 말을 반복하던 중 신호가 끊김. 이후 복원할 수 있던 기록에서 해당 실종자의 이름이 칠판에 쓰여있던 것을 확인",
        ...
        "$attempts번 실종자는 종소리가 울리기 시작한 이후로 신호가 끊김. 종소리와 학교 내부 환경의 변화에 대한 추가적인 조사 필요."
    };
    ...
    private void Update(){
        if(Input.GetKeyDown(KeyCode.P)){
            ArchiveLog arc = new ArchiveLog(cnt, "실종", $"{cnt}번 실종자는 복도를 돌아다니던 중 교장선생님과 마주친 후 신호가 끊김. 이후 자습실 구석 자리에서 피에 젖은 책 더미 외에는 해당 실종자와 관련된 흔적을 찾을 수 없었음.");
            AddArchiveLog(arc);
            cnt++;
        }
    }
