...
public class BoxMonster : BaseController
{
    Rigidbody rb;
    NavMeshAgent nav;
    ...
    protected override void UpdateIdle()
    {
        ...
        if (!patrolMonster)
          ...
          if (nav.remainingDistance <= nav.stoppingDistance)
            patrolMonster = false;
        if (Physics.CheckBox(transform.position+transform.forward*boxSize.x,boxSize,Quaternion.identity,playerMask)) // 감지 범위 내에 플레이어가 있다면 추격
        {
            Debug.Log("감지감지!!");
            //findParticle.Play();
    ...
    void OnDrawGizmos()
    {
        ...
        Gizmos.DrawCube(transform.position+transform.forward*boxSize.x, boxSize);
    }
}
